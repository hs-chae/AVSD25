...
def C_angle8(d):
    ...
    random.shuffle(tmp)
    d.entities.append(('C_angle8',
                       [tmp[0][0].label, tmp[0][1], tmp[1][0].label, tmp[1][1], tmp[2][0].label, tmp[2][1], tmp[0][2],
                        tmp[1][2], tmp[2][2]]))
...
def C_angle16(d):
    ...
    d.entities.append(('C_angle16', [O_1.label, O_2.label, O_3.label, O_4.label, col1, col2, col3,
                                     col4]))  # O_1 : ocute, O_2~4 : obtuse
...
def C_angle17(d):
    ...
    d.entities.append(('angle17', [O_1.label, O_2.label, O_3.label, O_4.label, col1, col2, col3,
                                   col4]))  # O_1 : ocute, O_2~4 : obtuse
...
def angle18(d):
    ...
    if color_type == 1:  # Every lines have the same color
        if random.choice([True, False]):
            ...
            wrapper_color = random.choice(d.usable_colors)
        ...
        wrapper_color, other_color = None, None  # Type 3: all colors are different
    ...
    for i in range(num_noisy_lines):
        ...
        while True:
            ...
            x, y = O.x + scale * vec[0], O.y + scale * vec[1]
            if assert_coord_in_range(x, y) and (angle - full_angle / nun_sections) ** 2 > 0.1 and (
                    angle - 2 * full_angle / nun_sections) ** 2 > 0.1 and (
                    angle - 3 * full_angle / nun_sections) ** 2 > 0.1:
                break
    ...
    if color_type == 3:
        ...
        d.entities.append(
            (f'angle18-{nun_sections}-3', colors_list + [color, color_candidates_1[:-2], color_candidates_2[:-1]]))
    elif color_type == 2:
        d.entities.append((f'angle18-{nun_sections}-2', [wrapper_color, other_color] + [
            f'{secting_endpoints_1[i].label}{secting_endpoints_2[i].label}' for i in
            range(len(secting_endpoints_2))] + [f'{line.point1.label}{line.point2.label}' for line in noisy_lines]))
    else:
        d.entities.append((f'angle18-{nun_sections}-1',
                           [f'{A1.label}{O.label}{B1.label}', f'{A1.label}{A2.label}', f'{B1.label}{B2.label}'] + [
                               f'{secting_endpoints_1[i].label}{secting_endpoints_2[i].label}' for i in
                               range(len(secting_endpoints_2))] + [f'{line.point1.label}{line.point2.label}' for line in
                                                                   noisy_lines]))
...
def angle19(d):
    ...
    if color_type == 1:  # All angles have same color
        ...
        d.entities.append(
            (f"angle19-{question_type}-1", [colors[0], answer_label, wrong_label, candidates1[:-2], candidates2[:-1]]))
        ...
        d.entities.append(
            (f"angle19-{question_type}-2", [answer_color, wrong_color, candidates1[:-2], candidates2[:-1]]))
        d.points = []
