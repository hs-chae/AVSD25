...
def nth_highest_index(arr, n):
    """
    Returns the index of the n-th highest number in the list `arr`.
    """
    sorted_indices = sorted(range(len(arr)), key=lambda i: arr[i], reverse=True)
    return sorted_indices[n-1]

def random_unique_letters(n):
    """
    Return a list (or string) of n distinct letters randomly chosen from
    the 52 English letters (a-z, A-Z).
    """
    all_letters = string.ascii_letters  # 'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ'
    selected_letters = random.sample(all_letters, n)
...
def reflection_2 (diagram: Diagram) : 
    
    def sample_thetas ( num_thetas= 2, 
                        max_theta = np.pi,
                       _delta_mean= 10 * np.pi / 180, _delta_std= 20 * np.pi / 180, _delta_min= 4.0 * np.pi / 180) : 
        # Generate 4 random increments from a Gamma distribution + constant. 
        deltas      = _delta_min + np.random.gamma(shape=(_delta_mean/_delta_std) ** 2, 
                                                   scale=(_delta_std ** 2)/_delta_mean, 
                                                   size= 1 + num_thetas)    
        deltas      = deltas / np.sum(deltas) * max_theta
        thetas      = np.cumsum(np.insert(deltas, 0, 0))[1:-1]
        return thetas 
    ...
    def add_random_reflective_polygon (diagram, center_pos ) : 
        
        num_thetas = np.random.choice([2,3,4])
        thetas_right = sample_thetas ( num_thetas= num_thetas, max_theta= np.pi )
        thetas_left = 2*np.pi - thetas_right[::-1]
        thetas  = np.concatenate( (thetas_right, thetas_left) )

        radials_right = np.random.uniform(50, 80, len(thetas_left))
        radials = np.concatenate( (radials_right, radials_right[::-1]))

        diagram = add_polygon (diagram, center_pos, thetas, radials)

        return diagram
    
    def add_random_polygon (diagram, center_pos) : 

        num_thetas = np.random.choice([4, 6, 8])
        thetas = sample_thetas (num_thetas= num_thetas, max_theta= np.pi*2)
        radials = np.random.uniform(50, 80, len(thetas))

        diagram = add_polygon (diagram, center_pos, thetas, radials)

        return diagram 
    
    answer_shape_idx = np.random.choice([0,1,2,3])
    
    labels = [] 

    for shape_idx in range(4) : 

        center_pos = np.array([125 + 250 * shape_idx, 500])
        label_pos = center_pos + np.array([0, 100])

        if shape_idx == answer_shape_idx : 
            diagram = add_random_polygon (diagram, center_pos)

        else : 
            diagram = add_random_reflective_polygon (diagram, center_pos)

        diagram, _point = add_fakepoint_at_position(diagram, label_pos, label=True)
        labels.append(_point.label)

    diagram.entities.append(('reflection_2', labels + [labels[answer_shape_idx]]))

    return diagram 


def reflection_3 (diagram: Diagram) : 
    ...
    diagram.entities.append(('reflection_3', labels + [labels[answer_shape_idx]]))

    return diagram 


def reflection_4 (diagram : Diagram) : 

    def sample_thetas_and_idx ( _delta_mean = 20 * np.pi / 180, _delta_std = 20 * np.pi / 180, _delta_min = 4.0 * np.pi / 180) : 
        # Generate 4 random increments from a Gamma distribution + constant. 
        deltas      = _delta_min + np.random.gamma(shape=(_delta_mean/_delta_std) ** 2, scale=(_delta_std ** 2)/_delta_mean, size=3)
        answer_idx  = np.random.choice(np.arange(len(deltas)+1))
        thetas      = np.cumsum(np.insert(deltas, 0, 0))
    ...
    def sample_R (_R_mean = 250, _R_std = 200, _R_max = 500, _R_min = 100 ) : 
        # Generate R from a Gamma distribution and reject values > 250
        while True :
            R = np.random.gamma(shape=(_R_mean/_R_std)**2, scale=(_R_std ** 2)/_R_mean)
            if R <= _R_max and R >= _R_min :
    ...
    line_thetas, real_idx = sample_thetas_and_idx() 
    R   = sample_R() 
    phi = np.random.uniform(-np.pi, np.pi)

    theta_star  = line_thetas[real_idx]
    pointO_pos  = np.random.normal(loc=500, scale=50, size=(2,))
    pointP_pos  = R * np.array([np.cos(theta_star + phi), np.sin(theta_star + phi)]) + pointO_pos
    pointQ_pos  = R * np.array([np.cos(theta_star - phi), np.sin(theta_star - phi)]) + pointO_pos

    def _sample_polygon_thetas ( num_thetas= 2, 
                        max_theta = np.pi,
                       _delta_mean= 10 * np.pi / 180, _delta_std= 20 * np.pi / 180, _delta_min= 4.0 * np.pi / 180) : 
        # Generate 4 random increments from a Gamma distribution + constant. 
        deltas      = _delta_min + np.random.gamma(shape=(_delta_mean/_delta_std) ** 2, 
    ...
    # Main Shape 
    diagram, pointO = add_point_at_position(diagram, pointO_pos)
    diagram, pointP = add_fakepoint_at_position(diagram, pointP_pos)
    diagram, pointQ = add_fakepoint_at_position(diagram, pointQ_pos)

    polygon_thetas  = _sample_polygon_thetas (num_thetas= np.random.choice([4, 6, 8]), max_theta= np.pi*2)
    polygon_radials = np.random.uniform(50, 80, len(polygon_thetas))
    diagram = add_polygon(diagram, pointP_pos, polygon_thetas, polygon_radials)
    diagram = add_polygon(diagram, pointQ_pos, 2 * line_thetas[real_idx] - polygon_thetas + np.pi, polygon_radials)

    # Candidate Lines 
    point1s = []
    for idx in range(len(line_thetas)): 
        _tangent        = np.array([np.cos(line_thetas[idx]), np.sin(line_thetas[idx])])
        point1_pos      =   450 * _tangent + pointO_pos
        point2_pos      = - 450 * _tangent + pointO_pos
        diagram, point1 = add_point_at_position(diagram, point1_pos)
        diagram, point2 = add_fakepoint_at_position(diagram, point2_pos, label=False)
        point1s.append(point1)
        ln12        = Line(point1, point2, '', infinite=True)
        diagram.lines.append(ln12)
        
    diagram.entities.append(('reflection_4', [pointO.label] + [each.label for each in point1s] + [point1s[real_idx].label]))
    
    return diagram 


def reflection_5 (diagram : Diagram) : 
    ...
    num_thetas = np.random.choice([2,3,4])
    thetas_right = _sample_polygon_thetas ( num_thetas= num_thetas, max_theta= np.pi )
    thetas_left = 2 * np.pi - thetas_right[::-1]
    thetas  = np.concatenate( (thetas_right, thetas_left) )

    radials_right = np.random.uniform(100, 160, len(thetas_left))
    radials = np.concatenate( (radials_right, radials_right[::-1]))

    diagram = add_polygon (diagram, pointO_pos, thetas + line_thetas[real_idx] + np.pi / 2, radials)
    ...
    diagram.entities.append(('reflection_5', [pointO.label] + [each.label for each in point1s] + [point1s[real_idx].label]))
    
    return diagram 

def reflection_6 (diagram : Diagram) : 
    
    # "Each shape in A is likely a reflection of the other with respect to a line, while the shapes in B are not. Now, among 1, 2, 3, and 4, in which figure are the shapes most likely to be reflections of each other with respect to a line?"
    # "Among <1>, <2>, <3>, and <4>, in which figure are the shapes most likely to be reflections of each other with respect to a line?"
    ...
    def add_line (diagram, point1_pos, point2_pos) : 
        diagram, point1 = add_fakepoint_at_position(diagram, point1_pos, label=False)
        diagram, point2 = add_fakepoint_at_position(diagram, point2_pos, label=False)
        line12 = Line(point1, point2, '', infinite=True)
        diagram.lines.append(line12)
        return diagram 

    center_pos_lst = np.array([
        [250, 750],
        [750, 750],
        [250, 250],
        [750, 250]
    ...
    answer_idx = random.choice([0,1,2,3])
    
    for idx in range(4) : 

        # -- add label 
        diagram, fake_point = add_fakepoint_at_position(
            diagram, 
            center_pos_lst[idx] + np.array([0, 50]), 
            label=True)
        label_lst.append(fake_point.label)

        # -- add center line 
        line_height = 200
        diagram = add_line(diagram, 
                           center_pos_lst[idx] + np.array([0, line_height/2]),
                           center_pos_lst[idx] - np.array([0, line_height/2]))
        
        # -- add shape pair
        distance = 100 # distance seperating each of the pair 
         
        num_thetas = np.random.choice([2,3,4,5])
        thetas = _sample_polygon_thetas ( num_thetas= num_thetas, max_theta= np.pi * 2 )
        radials = np.random.uniform(distance*0.3, distance*0.9, len(thetas))
        
        diagram = add_polygon (diagram, center_pos_lst[idx] - np.array([distance, 0]), 
                               thetas, radials)
        
        if idx == answer_idx : 
            diagram = add_polygon (diagram, center_pos_lst[idx] + np.array([distance, 0]), 
                                np.pi - thetas , radials)
        
        else : 
            random_distance = np.random.choice([distance * 0.7, distance, distance * 1.3])
            random_height_diff = np.random.uniform(-distance/2, distance/2)
            angle_perturb = np.random.uniform(-np.pi * 0.1, np.pi * 0.1)
            angle_mode = np.random.choice([-1, +1])

            diagram = add_polygon (diagram, center_pos_lst[idx] + np.array([random_distance, random_height_diff]), 
                                np.pi - angle_mode * thetas + angle_perturb , radials)

    diagram.entities.append(('reflection_6', label_lst + [label_lst[answer_idx]]))
...
#     width = np.random.uniform(500, 900)
#     height = np.random.uniform(500, 900)

#     x1 = np.random.uniform(10, 1000 - width)
#     y1 = np.random.uniform(10, 1000 - height)

#     # generate table outlines 
#     for idx in range(9 + 1) : 
#         diagram, fakepoint_1 = add_fakepoint_at_position(diagram, 
#                                                          point_pos= np.array([x1, y1 + idx * height/9]), label=False)
#         diagram, fakepoint_2 = add_fakepoint_at_position(diagram, 
#                                                          point_pos= np.array([x1 + width, y1 + idx * height/9]), label=False)
#         line_12 = Line(point1= fakepoint_1, point2= fakepoint_2, label='')
#         diagram.lines.append(line_12)

#     for idx in range(9+1) : 
#         diagram, fakepoint_1 = add_fakepoint_at_position(diagram, 
#                                                          point_pos= np.array([x1 + idx * width/9, y1]), label=False)
#         diagram, fakepoint_2 = add_fakepoint_at_position(diagram, 
#                                                          point_pos= np.array([x1 + idx * width/9, y1+height]), label=False)
#         line_12 = Line(point1= fakepoint_1, point2= fakepoint_2, label='')
#         diagram.lines.append(line_12)

#     diagram, fakepoint_1 = add_fakepoint_at_position(diagram, point_pos= np.array([x1 + width/2, 10]), label=False)
#     diagram, fakepoint_2 = add_fakepoint_at_position(diagram, point_pos= np.array([x1 + width/2, 990]), label=False)
#     line_12 = Line(point1= fakepoint_1, point2= fakepoint_2, label='')
#     diagram.lines.append(line_12)

#     def add_colored_square (diagram, idx_x, idx_y, c, x1= x1, y1= y1, width= width, height= height) : 
#         diagram = add_coloredpolygon_cartesian(diagram, 
#                                                center_pos = np.array([
#                                                    x1 + (idx_x + 0.5) * width / 9,
#                                                    y1 + (idx_y + 0.5) * height / 9
#                                                ]), 
...
#     diagram = add_colored_square(diagram, 1, 0, 'b')
#     diagram = add_colored_square(diagram, 7, 0, 'b')

#     diagram = add_colored_square(diagram, 2, 3, 'r')
#     diagram = add_colored_square(diagram, 7, 5, 'r')

#     diagram = add_colored_square(diagram, 5, 8, 'g')
#     diagram = add_colored_square(diagram, 1, 3, 'g')

#     diagram = add_colored_square(diagram, 8, 5, 'y')
#     diagram = add_colored_square(diagram, 3, 3, 'y')

#     diagram.entities.append(('reflection_6', []))
#     return diagram 
