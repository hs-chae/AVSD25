...
def plot_point(ax, tc, label = '', coord = (random.randint(0, 1000),random.randint(0, 1000)), color = 'black', fixing_color = True):
    ...
    x_vec, y_vec = l*np.cos(ang), l*np.sin(ang)
    # Adding a label "X" next to the point with random vertical alignment and color
    tc.append(x_coord + x_vec, y_coord + y_vec, label, verticalalignment=random_vertical_alignment, color='black')
...
def plot_angle(ax, tc, line1, angle, intersection, color='black', label = None, length = None):
    start_angle = np.arctan2(line1.point2.y - intersection.y, line1.point2.x - intersection.x)
    start_angle_deg = np.degrees(start_angle)
    if length is not None:
        radius = 0.15*length
    else:
        radius = 0.15 * np.sqrt((line1.point2.x - intersection.x) ** 2 + (line1.point2.y - intersection.y) ** 2)
    ...
    label_x, label_y = intersection.coord + 1.25 * radius * np.array([np.cos(start_angle+ 0.5 * angle), np.sin(start_angle + 0.5 * angle)])
    ax.add_patch(arc_patch)

    tc.append(label_x, label_y, f"{label}", color='black')
...
def plot_diagram(ax, tc, diagram):
    ...
    for ang in diagram.angles:
        line1, angle_value, intersection, color, ang_label, length = ang
        plot_angle(ax, tc, line1, angle_value, intersection, label = ang_label, color = color, length = length)
