...
def plot_point(ax, tc, label='', coord=(random.randint(0, 1000), random.randint(0, 1000)), color='black',
               fixing_color=True):
    ...
    if color != "transparent":
        ...
        ax.plot(x_coord, y_coord, marker='o', color=color, markersize=2)  # 'o' stands for circle marker
...
def plot_line(ax, tc, label, point1: Point, point2: Point, color='black', infinite=False, tickmarks=0, dotted=False,
              fixing_color=True):
    # Coordinates for the free point X
    x1, y1 = point1.coord
    x2, y2 = point2.coord
    ...
    if not infinite:
        ...
        tc.append((x1 + x2) / 2 + 20, (y1 + y2) / 2, label, verticalalignment=random_vertical_alignment,
                color='black')
    else:
...
def plot_curve(ax, tc, x, y, t_start=0, t_end=1000, color='black'):
...
def plot_angle(ax, tc, line1, angle, intersection, color='black', label=''):
    ...
    # tc.append(label_x, label_y, f"{int(label_angle)}°", fontsize=12, color='black')
...
def plot_diagram(ax, tc, diagram):
    ...
    for filled_circle in diagram.filled_circles:
        plot_filled_circle(ax, tc, center=filled_circle.center, radius=filled_circle.radius, label=filled_circle.label,
    ...
    for line in diagram.lines:
        plot_line(ax, tc, label=line.label, point1=line.point1, point2=line.point2, infinite=line.infinite,
    ...
    for curve in diagram.curves:
        plot_curve(ax, tc, curve.x, curve.y, color=curve.color)

    for ang in diagram.angles:
        line1, line2, intersection, color, ang_label = ang
        plot_angle(ax, tc, line1, line2, intersection, color = color,  label=ang_label)
