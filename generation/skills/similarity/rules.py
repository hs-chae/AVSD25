...
def similarity_2 (diagram : Diagram) : 
    ...
    shape_functions = {
        ...
        'hexagon': lambda: sample_polygon_shape(shape_n=6, return_shape_name=True), 
    ...
    transformations = {
        "scale_radial": lambda: scale_subset_of_radials(ref_radials, ref_thetas),
        "scale_x": lambda: scale_cartesians(ref_radials, ref_thetas, scale_x=True, scale_y=False),
        "scale_y": lambda: scale_cartesians(ref_radials, ref_thetas, scale_x=False, scale_y=True)
    }

    center_positsions = np.array([np.array([125 + 250 * idx, 500]) for idx in range(4)])

    shape_info_lst = []
    similar_pairs = random.sample(range(4), 2)  
    shape = random.choice(list(shape_functions.keys()))
    ref_radials, ref_thetas, ref_name = shape_functions[shape]()
    ref_radials *= 100 / np.max(ref_radials)  

    shape_info_lst = [(ref_radials.copy(), ref_thetas.copy(), ref_name) for _ in range(4)]

    labels = []
    for idx in range(4):
        ref_radials, ref_thetas, ref_name = shape_info_lst[idx]
        ref_radials, ref_thetas = similarity_transform(ref_radials, ref_thetas)

        if ref_name in ['circle', 'heart'] : # curves
            _positions = transformations[random.choice(["scale_x", "scale_y"])]() if idx not in similar_pairs else scale_cartesians(ref_radials, ref_thetas, scale_x=False, scale_y=False)
        else : 
            _positions = transformations[random.choice(list(transformations.keys()))]() if idx not in similar_pairs else scale_cartesians(ref_radials, ref_thetas, scale_x=False, scale_y=False)

        diagram = add_polygon_cartesian(diagram, center_pos= center_positsions[idx], positions= _positions, return_polygon=False )
        diagram, _fakepoint = add_fakepoint_at_position(diagram, center_positsions[idx] + np.array([0, 200]), label=True)
        labels.append(_fakepoint.label)

    diagram.entities.append(('similarity_2', labels + [labels[similar_pairs[0]], labels[similar_pairs[1]]]))
    return diagram 


def similarity_3 (diagram : Diagram) : 
    ...
    shape_info_lst = []
    similar_pairs = random.sample(range(4), 3)  
    anomaly_idx = list(set(range(4)) - set(similar_pairs))[0]
    ...
    labels = []
    for idx in range(4):
        ref_radials, ref_thetas, _ = shape_info_lst[idx]
        ref_radials, ref_thetas = similarity_transform(ref_radials, ref_thetas)
        if ref_name in ['circle', 'heart'] : # curves
            _positions = transformations[random.choice(["scale_x", "scale_y"])]() if idx not in similar_pairs else scale_cartesians(ref_radials, ref_thetas, scale_x=False, scale_y=False)
        else : 
    ...
    diagram.entities.append(('similarity_3', labels + [labels[anomaly_idx]]))
    return diagram 

def similarity_4 (diagram : Diagram) : 
    ...
    center_positsions = np.array([np.array([100 + 200 * idx, 500]) for idx in range(5)])

    shape_info_lst = []
    answer_idx = random.choice(range(1, 5))
    similar_pairs = [0, answer_idx]

    shape = random.choice(list(shape_functions.keys()))
    ref_radials, ref_thetas, ref_name = shape_functions[shape]()
    ref_radials *= 70 / np.max(ref_radials)  

    shape_info_lst = [(ref_radials.copy(), ref_thetas.copy(), ref_name) for _ in range(5)]
    ...
    diagram.entities.append(('similarity_5', [ref_name] + labels + [labels[answer_idx-1]]))
    return diagram 

def similarity_5 (diagram : Diagram) : 
    ...
    shape_info_lst = []
    similar_pairs = random.sample(range(1, 5), 3) + [0]  
    anomaly_idx = list(set(range(5)) - set(similar_pairs))[0]
    ...
    diagram.entities.append(('similarity_5', [ref_name] + labels + [labels[anomaly_idx-1]]))
    return diagram 

def similarity_6 (diagram : Diagram) : 
    ...
    num_candidates = 3 
    answer_candidate_idx = random.choice(range(num_candidates))
    x_coordinates = np.array( [ (2 * _ + 1) * 1000 / 6 for _ in range(num_candidates) ] )
    ...
    for candidate_idx in range(num_candidates) : 
        ...
        _positions = scale_cartesians(ref_radials, ref_thetas, scale_x=False, scale_y=False)
        diagram = add_polygon_cartesian(diagram, center_pos= np.array([x_coordinates[candidate_idx], 250]), positions= _positions, return_polygon=False )
        ...
        if ref_name in ['circle', 'heart'] : # curves
            _positions = transformations[random.choice(["scale_x", "scale_y"])]() if candidate_idx != answer_candidate_idx else scale_cartesians(ref_radials, ref_thetas, scale_x=False, scale_y=False)
        else : 
            _positions = transformations[random.choice(list(transformations.keys()))]() if candidate_idx != answer_candidate_idx else scale_cartesians(ref_radials, ref_thetas, scale_x=False, scale_y=False)
        diagram = add_polygon_cartesian(diagram, center_pos= np.array([x_coordinates[candidate_idx], 500]), positions= _positions, return_polygon=False )

        diagram, _fakepoint = add_fakepoint_at_position(diagram, np.array([x_coordinates[candidate_idx], 750]), label=True)
        labels.append(_fakepoint.label)


    diagram.entities.append(('similarity_6', labels + [labels[answer_candidate_idx]]))
    return diagram 
