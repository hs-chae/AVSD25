...
def plot_point(ax, label='', coord=(random.randint(0, 1000), random.randint(0, 1000)), color='black',
               fixing_color=True):
    ...
    ax.text(x_coord + 5, y_coord, label, fontsize=12, verticalalignment=random_vertical_alignment, color='black')


def plot_line(ax, label, point1: Point, point2: Point, color='black', infinite=False, tickmarks=0, dotted=False,
              fixing_color=True):
    # Coordinates for the free point X
    x1, y1 = point1.coord
    x2, y2 = point2.coord
    ...
    if not infinite:
        ...
        if dotted:
            ...
            ax.plot([x1, x2], [y1, y2], color=color)  # 'o' stands for circle marker
        ax.text((x1 + x2) / 2 + 20, (y1 + y2) / 2, label, fontsize=12, verticalalignment=random_vertical_alignment,
                color='black')
    else:
...
def plot_circle(ax, label, center, radius, color='black'):
    # Coordinates for the free point X
    x, y = center.coord
...
def plot_triangle(ax, point1, point2, point3, diagram=Diagram(), mark_points=False,
...
def plot_perpendicularity(ax, line1, line2, intersection):
    ...
    ax.plot([x5 + norm_direction2[0], x5 + norm_direction1[0] + norm_direction2[0]],
            [y5 + norm_direction2[1], y5 + norm_direction1[1] + norm_direction2[1]], color='black')
    ax.plot([x5 + norm_direction1[0], x5 + norm_direction2[0] + norm_direction1[0]],
            [y5 + norm_direction1[1], y5 + norm_direction1[1] + norm_direction2[1]], color='black')


def plot_curve(ax, x, y, t_start=0, t_end=1000, color='black'):
    parameter = np.linspace(t_start, t_end, 1000)
    ax.plot(x, y, color=color)


def plot_arc(ax, line1, angle, color='black', label=''):
    """
...
def plot_angle(ax, line1, angle, intersection, color='black', label=''):
    # print(f"plot_angle: angle: {angle}, color: {color}, label: {label}")
    # line 1 angle
    start_angle_deg = np.degrees(np.arctan2(line1.point2.y - intersection.y, line1.point2.x - intersection.x))
    radius = 0.2 * np.sqrt((line1.point2.x - intersection.x) ** 2 + (line1.point2.y - intersection.y) ** 2)
    arc_patch = patches.Arc(
        intersection.coord,
        width=2 * radius,  # full width of the circle
        height=2 * radius,  # full height of the circle
        angle=0,  # rotation of the entire ellipse (0 = no rotation)
        theta1=start_angle_deg,  # start angle
        theta2=start_angle_deg + np.degrees(angle),  # end angle
        linewidth=2,
        color=color
    )
    ...
    # ax.text(label_x, label_y, f"{int(label_angle)}°", fontsize=12, color='black')


def plot_polygon(ax, points, edge_color='black', fill_color='white', label=''):
    ax.add_patch(
    ...
    ax.text(midpoint_x, midpoint_y, label, fontsize=12, verticalalignment='center', color='black')


def plot_filled_circle(ax, center, radius, label='', edge_color='black', fill_color='yellow'):
    ax.add_patch(patches.Circle(center, radius, edgecolor=edge_color, facecolor=fill_color, linewidth=2))

    ax.text(center[0], center[1], label, fontsize=12, verticalalignment='center', color='black')
...
def plot_diagram(ax, diagram):
    ...
    for plgn in diagram.polygons:
        plot_polygon(ax, points=plgn.points, edge_color=plgn.edge_color, fill_color=plgn.fill_color, label=plgn.label)

    for filled_circle in diagram.filled_circles:
        plot_filled_circle(ax, center=filled_circle.center, radius=filled_circle.radius, label=filled_circle.label,
                           edge_color=filled_circle.edge_color, fill_color=filled_circle.fill_color)



    for line in diagram.lines:
        plot_line(ax, label=line.label, point1=line.point1, point2=line.point2, infinite=line.infinite,
                  tickmarks=line.tickmarks, dotted=line.dotted, color=line.color)
    ...
    for circle in diagram.circles:
        ...
        plot_circle(ax, label='', center=circle.center, radius=circle.radius, color=circle.color)
    ...
    for curve in diagram.curves:
        plot_curve(ax, curve.x, curve.y, color=curve.color)

    for ang in diagram.angles:
        line1, line2, intersection, color, ang_label = ang
        plot_angle(ax, line1, line2, intersection, color = color,  label=ang_label)

    for point in diagram.points:
        plot_point(ax, label=point.label, coord=point.coord, color=point.color)
