...
def create_diagram(num_entities):
    diagram = Diagram()
    while True:
        diagram, rule = create_tree(diagram)
        if rule == 'abs_position_object' or len(diagram.entities) >= num_entities or len(diagram.points)>12 or len(diagram.steps) > 6: #or indx > len(diagram.entities)*1.5
            break
    return diagram
