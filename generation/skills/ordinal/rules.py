...
def ordinal_1 (diagram : Diagram) : 
    ...
    for idx in range(len(shape_lst)) : 
        ...
        if shape_lst[idx] == 'hexagon' :     
            diagram = add_regular_polygon(diagram, center_pos=loc, radius= scale, n=6)

        elif shape_lst[idx] == 'rectangle' : 
            diagram = add_regular_polygon(diagram, center_pos=loc, radius= scale, n=4)

        elif shape_lst[idx] == 'circle' : 
            diagram = add_regular_polygon(diagram, center_pos=loc, radius= scale, n=1000)

        elif shape_lst[idx] == 'triangle' : 
            diagram = add_regular_polygon(diagram, center_pos=loc, radius= scale, n=3)

        elif shape_lst[idx] == 'pentagon' : 
            diagram = add_regular_polygon(diagram, center_pos=loc, radius= scale, n=5)
...
def ordinal_2 (diagram : Diagram) : 
    ...
    label_lst = [] 
    for idx in range(num_labels) : 
        loc = np.array([np.random.uniform(400, 600), 
                        1000 - (idx+1) * (1000/(num_labels+1))])
        diagram, fpoint = add_fakepoint_at_position(diagram, loc, label=True)
        label_lst.append(fpoint.label)

    idx_to_ask      = np.random.choice([0,1,2,3,4])
    from_the_bottom = ['first', 'second', 'third', 'fourth', 'fifth']
    
    diagram.entities.append(('ordinal_2', [from_the_bottom[idx_to_ask], label_lst[idx_to_ask]]))
    return diagram 

def ordinal_3 (diagram : Diagram)  :
    
    "There are three labeled points on this line graph. Choose the point that is in the second-highest position. The answer should be one of A, B, or C."
    "There are three labeled points on this line graph. Choose the point that is in the <1>-highest position. The answer should be one of <2>, <3>, or <4>."
    
    num_points = 3 
    proposals = np.random.uniform(200, 800, size=(10,num_points,2))
    coord_lst = sample_repulsive_cartesian(proposals)

    point_lst = [] 
    label_lst = [] 
    for idx in range(num_points) : 
        diagram, point = add_point_at_position(diagram, point_pos= coord_lst[idx], label=True)
        point_lst.append(point)
        label_lst.append(point.label)
        if idx >= 1 : 
            ln = Line(point1=point_lst[idx-1], point2=point_lst[idx], label='', infinite=False)
            diagram.lines.append(ln)

    order_to_ask      = np.random.choice([0,1,2])
    order_name = ['first', 'second', 'third', 'fourth', 'fifth']
    answer_idx = nth_highest_index(arr=coord_lst[:, 1], n=order_to_ask)
    answer_label = label_lst[answer_idx]

    diagram.entities.append(('ordinal_3', [order_name[order_to_ask]] + label_lst + [answer_label]))
    return diagram 

def ordinal_4 (diagram : Diagram)  :

    "In the image, there are three labeled triangles with different sizes. Which is the label of the third smallest triangle? The answer should be one of ‘A’, ‘B’, or ‘C’.",
    "In the image, there are three labeled triangles with different sizes. Which is the label of the <1> smallest triangle? The answer should be one of <2>, <3>, or <4>."

    num_shapes = 3
    proposals = np.random.uniform(200, 800, size=(10,num_shapes,2))
    center_positions = sample_repulsive_cartesian (proposals= proposals, pair_potential='soft_coulomb')
    ...
    shape_radials, shape_thetas, shape_name = shape_functions[shape_name]()
    ...
    for center_pos, (shape_radials, shape_thetas, shape_name) in zip(center_positions, shape_info_lst) : 
        ...
        _scale = np.random.uniform(0.2, 1 - epsilon) * min_distance / np.max(shape_radials) 
        scale_lst.append(_scale)

        diagram = add_polygon(diagram, center_pos, 
                              thetas    = shape_thetas+np.random.uniform(0,np.pi*2), 
                              radials   = shape_radials * _scale, 
                              alpha     = np.random.uniform(0.5, 1.0))
        
        diagram, _fpoint = add_fakepoint_at_position(diagram, center_pos, label=True)
        label_lst.append(_fpoint.label)
        
    order_to_ask = np.random.choice([0,1,2])
    order_name = ['first', 'second', 'third', 'fourth', 'fifth']

    answer_idx = nth_highest_index(arr=-np.array(scale_lst), n=order_to_ask)
    answer_label = label_lst[answer_idx]

    diagram.entities.append(('ordinal_4', [order_name[order_to_ask]] + label_lst + [answer_label]))
    return diagram 

def ordinal_5 (diagram : Diagram) : 
    
    "There are several letters in the image. What is the letter that is <1> from the above? You should also consider its case."
    
    num_letters = np.random.choice([3, 4, 5, 6, 7, 8, 9, 10])
    letter_lst = random_unique_letters(n = num_letters)

    x_coord = np.random.uniform(100, 900)
    y_coord_lst = np.linspace(100, 900, num_letters)

    for idx in range(num_letters) : 
        diagram, _ = add_fakepoint_at_position_given_label(diagram,
                                                           point_pos=np.array([x_coord, y_coord_lst[idx]]),
                                                           given_label=letter_lst[idx])
    
    order_to_ask = np.random.choice(range(num_letters))
    order_name = ['first', 'second', 'third', 'fourth', 'fifth', 'sixth', 'seventh', 'eighth', 'nineth', 'tenth']

    answer_idx = nth_highest_index(arr=np.array(y_coord_lst), n=order_to_ask)
    answer_letter = letter_lst[answer_idx]
    
    diagram.entities.append(('ordinal_5', [order_name[order_to_ask], answer_letter]))
    return diagram 

def ordinal_6 (diagram : Diagram) : 
    
    "There is a table with distinct numbers in the image. Find the location of number 7. Answer in ”(row, column)” format. For example, 4 is at (1, 1) and 2 is at (1, 3)."
    
    "There is a table with distinct letters in the image. Find the location of letter <1>. Answer in ”(row, column)” format. For example, <3> is at <2> and <4> is at <3>.",
    
    positions = [(row, col) for row in range(3) for col in range(3)]
    # [(0, 0), (0, 1), (0, 2), (1, 0), (1, 1), (1, 2), (2, 0), (2, 1), (2, 2)]
    letters = random_unique_letters(n = 9)
    
    selected_indices = random.sample(range(9), 3)
    answer_idx = selected_indices[0]
    eg1_idx = selected_indices[1]
    eg2_idx = selected_indices[2]
    ...
    for idx in range(4) : 
        diagram, fakepoint_1 = add_fakepoint_at_position(diagram, 
                                                         point_pos= np.array([x1, y1 + idx * height/3]), label=False)
        diagram, fakepoint_2 = add_fakepoint_at_position(diagram, 
                                                         point_pos= np.array([x1 + width, y1 + idx * height/3]), label=False)
        line_12 = Line(point1= fakepoint_1, point2= fakepoint_2, label='')
        diagram.lines.append(line_12)

    for idx in range(4) : 
        diagram, fakepoint_1 = add_fakepoint_at_position(diagram, 
                                                         point_pos= np.array([x1 + idx * width/3, y1]), label=False)
        diagram, fakepoint_2 = add_fakepoint_at_position(diagram, 
                                                         point_pos= np.array([x1 + idx * width/3, y1+height]), label=False)
        line_12 = Line(point1= fakepoint_1, point2= fakepoint_2, label='')
        diagram.lines.append(line_12)
    ...
    for position, letter in zip(positions, letters) : 
        row, col = position
        diagram, fakepoint = add_fakepoint_at_position_given_label(diagram,
                                                                point_pos= np.array([x1 + (col + 1/2)* width/3, 
                                                                                        y1 + height - (row + 1/2)* height/3]),
                                                                given_label=letter)
    
    diagram.entities.append(('ordinal_6', [f'{(positions[answer_idx][0]+1, positions[answer_idx][1]+1)}', 
                                           f'{(positions[eg1_idx][0]+1, positions[eg1_idx][1]+1)}', 
                                           letters[eg1_idx],
                                           f'{(positions[eg2_idx][0]+1, positions[eg2_idx][1]+1)}', 
                                           letters[eg2_idx],
                                           letters[answer_idx]]))
    return diagram 
...
# "question": "Choose the phrase in parentheses that correctly describes the image. In the image, the green hexagon and the orange shape {do not meet/are tangent to each other/overlap}.",
def overlap_1 (diagram : Diagram) : 
    ...
    diagram.entities.append(('overlap_1', [answer]))
