...
def plot_point(ax, tc, label='', coord=(random.randint(0, 1000), random.randint(0, 1000)), 
    ...
    # tc.append(x_coord + 20, y_coord, label, fontsize=RANDOM_FONT_SIZE, style=RANDOM_FONT_STYLE, color=font_color)
...
def plot_fakepoint(ax, tc, label = '', coord = (random.randint(0, 1000),random.randint(0, 1000)), 
    ...
    # tc.append(x_coord, y_coord, label, fontsize=RANDOM_FONT_SIZE, verticalalignment=random_vertical_alignment, color=font_color)
...
def plot_line(ax, tc, label, point1 : Point, point2 : Point, color = random.choice(colors.candidates), infinite = False, tickmarks = 0, dotted = False, 
    ...
    if not infinite:
        ...
        # tc.append((x1 + x2)/2 + 20, (y1 + y2)/2, label, fontsize=12, verticalalignment=random_vertical_alignment, color='black')
        ...
        # tc.append((x1 + x2) / 2, (y1 + y2) / 2, label, fontsize=12, verticalalignment=random_vertical_alignment, color='black')
