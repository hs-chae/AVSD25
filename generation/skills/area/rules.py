...
# Function to sample a pi shape 
def sample_pi_shape(theta_start, theta_end, num_points = 1000) : 
    """
    Generates a pi shape with angle theta
    ...
    Returns:
        tuple: shape_radials, shape_thetas
    """
    shape_thetas = np.linspace(theta_start, theta_end, num_points)
    shape_radials = np.ones(num_points)
    shape_radials[[0, -1]] = 0.0
    return shape_radials, shape_thetas
...
def add_pi_graph ( diagram, angle_lst, color_lst, label_lst, ref_angle, center, radius, radius_for_label_ratio=0.7) :     
        
    current_angle = ref_angle

    for angle, color, label in zip(angle_lst, color_lst, label_lst):
        radials, thetas = sample_pi_shape(current_angle, current_angle + angle)
        diagram = add_coloredpolygon(
            diagram=diagram,
            center_pos=center,
            thetas=thetas,
            radials=radials * radius,
            color=color
        )
        diagram, _ = add_fakepoint_at_position_given_label(
            diagram=diagram,
            point_pos= center + np.array([
                np.cos(current_angle + angle / 2), 
                np.sin(current_angle + angle / 2)
                ]) * radius * radius_for_label_ratio ,
            given_label=label
        )
...
def area_1_ref (diagram, n, rule_name='area_1_?'):    
    [
        "In the image, there is a pie graph with $n categories. Which category has the largest ratio? Choose from <1>, <2>, ..., and <$n>.",
        "Category <$(n+1)> has the largest ratio."
    ]
    
    ## --- Parameters ----
    colors_avail = ['red', 'orange', 'yellow', 'green', 'blue', 'purple']
    labels_avail = list(string.ascii_uppercase)  # ['A', 'B', ..., 'Z']
    if n > len(colors_avail):
        raise ValueError(f"Not enough colors available for n={n}.")
    if n > len(labels_avail):
        raise ValueError(f"Not enough labels available for n={n}.")
    radius_range = (100, 480)  # (min_radius, max_radius)

    ## --- Sample ----
    angle_lst = 2 * np.pi * np.random.dirichlet(np.ones(n))
    color_lst = np.random.choice(colors_avail, size=n, replace=False)
    label_lst = np.random.choice(labels_avail, size=n, replace=False)
    radius = np.random.uniform(*radius_range)
    center = np.random.uniform(radius, 1000 - radius, 2)
    ref_angle = np.random.uniform(0, 2 * np.pi)
    radius_for_label_ratio = np.random.uniform(0.5, 1.2)

    ## --- Transform to a Problem  ---
    diagram = add_pi_graph(diagram, angle_lst=angle_lst, color_lst=color_lst, label_lst=label_lst,
                           ref_angle=ref_angle, center=center, radius=radius, radius_for_label_ratio=radius_for_label_ratio)
    answer_idx = np.argmax(angle_lst)
    diagram.entities.append((rule_name, list(label_lst) + [label_lst[answer_idx]]))
    
    return diagram

def area_1_1 (diagram):
    [
    ...
    return area_1_ref(diagram, n=4, rule_name='area_1_1')

def area_1_2 (diagram):
    [
    ...
    return area_1_ref(diagram, n=5, rule_name='area_1_2')
    
def area_1_3 (diagram):
    [
    ...
    return area_1_ref(diagram, n=3, rule_name='area_1_3')

def area_1_4 (diagram):
    [
    ...
    return area_1_ref(diagram, n=6, rule_name='area_1_4')

def area_1_5 (diagram):
    [
    ...
    return area_1_ref(diagram, n=2, rule_name='area_1_5')

def area_2_1 (diagram):    
    [
    ...
    indices_to_ask = random.sample(range(n), k=2)

    ## --- Transform to a Problem  ---
    diagram = add_pi_graph(diagram, angle_lst=angle_lst, color_lst=color_lst, label_lst=label_lst,
                           ref_angle=ref_angle, center=center, radius=radius)
    
    answer = 'larger' if angle_lst[indices_to_ask[0]] > angle_lst[indices_to_ask[1]] else 'smaller'
    diagram.entities.append(('area_2_1', [f'{label_lst[indices_to_ask[0]]}', f'{label_lst[indices_to_ask[1]]}', answer]))
    
    return diagram

def area_2_2 (diagram):    
    [
    ...
    indices_to_ask = random.sample(range(n), k=3)

    ## --- Transform to a Problem  ---
    diagram = add_pi_graph(diagram, angle_lst=angle_lst, color_lst=color_lst, label_lst=label_lst,
                           ref_angle=ref_angle, center=center, radius=radius)
    
    answer_1 = 'larger' if angle_lst[indices_to_ask[0]] > angle_lst[indices_to_ask[1]] else 'smaller'
    answer_2 = 'larger' if angle_lst[indices_to_ask[1]] > angle_lst[indices_to_ask[2]] else 'smaller'
    diagram.entities.append(('area_2_2', [f'{label_lst[indices_to_ask[0]]}', f'{label_lst[indices_to_ask[1]]}', f'{label_lst[indices_to_ask[2]]}', answer_1, answer_2]))
    
    return diagram

rules = []
rules += [area_1_1, area_1_2, area_1_3, area_1_4, area_1_5]
rules += [area_2_1]
rules += [area_2_2]
